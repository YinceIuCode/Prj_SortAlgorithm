#include "sorting.h"
//  Selection Sort, Insertion Sort, Binary Insertion Sort, Bubble Sort, Shaker Sort, Shell Sort, Heap Sort, Merge Sort, Quick Sort, Counting Sort, Radix Sort, and Flash Sort.
vector<string> algos = {"selection-sort", "insertion-sort", "binary-insertion-sort", "bubble-sort", "shaker-sort", "shell-sort", "heap-sort", "merge-sort", "quick-sort", "counting-sort", "radix-sort", "flash-sort"};
...
void processSort(vector<string> algs, string input_file, string output_params, int flag = 0) {
    ...
    for (int i = 0; i < algs.size(); ++i) {
        ...
        if (alg == "binary-insertion-sort") {
            record = getRecord(a, n, binaryinsertionSort, binaryinsertionSort);
        }
        if (alg == "bubble-sort") {
